...
const DEFAULT_BASE = 'http://10.16.206.1:3000';
const baseUrl = (globalThis as any).H2O_API_URL || DEFAULT_BASE;
...
function mapOrder(o: any) {
  ...
  return {
    ...
    items: (o.items || []).map((it: any) => ({ product: it.product || it.productId || it.productId?._id, productId: (it.product?._id) || it.productId || it.productId, quantity: it.quantity || it.qty || 1, raw: it })),
    status: o.status,
    address: o.address,
    paymentMethod: o.paymentMethod,
    createdAt: o.createdAt || o.placedAt,
    raw: o,
  };
}
...
export const api = {
  ...
  async register(payload: { name: string; email?: string; phone?: string; password: string }) {
    ...
    // Some setups may not issue a token on register; return a fake token placeholder and user mapping
    return { token: 'local-register-token', user: { id: res._id || res.id, name: res.name, email: res.email, phone: res.phone } };
  },
