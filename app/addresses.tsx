...
export default function Addresses() {
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <Button title="Add address" onPress={async () => { if (user) { await api.addAddress({ userId: user.id, label: 'Home', text: '123 Main St' }); const a = await api.getAddresses(user.id); setAddrs(a); } }} />
    </ThemedView>
  );
}
